...
\tikzset{
  pics/llnode/.style args={#1/#2}{
    code = {
      ...
      \node[node label,fit={(#1 prev) (#1 prev label) (#1 next) (#1 next label) (#1 data) (#1 data label)},label={[label distance=0.25cm]#2\strut}] (#1) {};
    }
  }
}
...
\begin{tikzpicture}
  ...
  \pic at (2,0) {llnode=new_node/{new_node}};

  \pic at (4,0) {llnode=new_node2/{}};


  \node[DOTS] at (-1.75, 1.25) (prev NULL) {$\cdots$};

  \node[DOTS] at (5.75, 0) (next NULL) {$\cdots$};

  \node[data box] at (0,-2.5) (Hello) {Hello\textbackslash0};
  \node[data box] at (2,-2.5) (tothe) {to_the\textbackslash0};
  \node[data box] at (4,-2.5) (World) {World\textbackslash0};

  \draw[link] (node_ptr data) -- (Hello);
  \draw[new link] (new_node data) -- (tothe);
  \draw[link] (new_node2 data) -- (World);

  \draw[link] (node_ptr prev) -- (prev NULL);
  % \draw[link] (new_node next) -- (next NULL);

  \draw[new link] (new_node prev) -- (node_ptr.east |- new_node prev);
  \draw[new link] (node_ptr next) -- (new_node.west |- node_ptr next);
  \draw[new link] (new_node next) -- (new_node2.west |- new_node next);
  \draw[link] (new_node2 next) -- (next NULL);
  \draw[new link] (new_node2 prev) -- (new_node.east |- new_node2 prev);
\end{tikzpicture}
