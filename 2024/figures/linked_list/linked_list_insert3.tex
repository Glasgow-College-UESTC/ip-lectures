...
\begin{tikzpicture}
  ...
  \draw (node3 next)  ++(0,-1) node (NULL_PTR) {};

  \draw[link] (node3 next) -- (node3 next-|node4.west);
  ...
  \draw[arrows=Latex-] (node3.south) -- (NULL_PTR -| node3) node (node3 arrow) {};

  \draw[arrows=Latex-] (node4.south) -- (NULL_PTR -| node4) node (node4 arrow) {};

  \node[PTR, below, text=blue] at (node3 arrow) (NODE) {node\_ptr};
